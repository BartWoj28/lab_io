...
public class MemProductDao implements ProductDao {
    ...
    @Override
    public List<Product> findByProducer(Producer d) {
       return SampleData.products.stream().filter(m -> m.getProducer() == d).collect(Collectors.toList());
    }

    @Override
    public List<Product> findByStore(Store c) {
        return SampleData.products.stream().filter(m -> m.getStores().contains(c)).collect(Collectors.toList());
    }
